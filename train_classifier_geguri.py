...
from geguri.model import MultiTaskResNet, MultiTaskEfficientNet, load_geguri
...
from misc.loss import lovasz_hinge, LabelSmoothingLoss
...
def main():
    ...
    args.out = os.path.join(args.out, f'classifier-aux-{datetime.now().strftime("%Y%m%d-%H%M%S")}')
    ...
    if args.checkpoint:
        ...
        load_geguri(model, state_dict['state_dict'], args.msd)
...
def train(args, labeled_trainloader, test_loader,
    ...
    if args.label_smoothing:
        cross_entropy_loss = LabelSmoothingLoss()
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        losses_mask_ce = AverageMeter()
        losses_lovasz = AverageMeter()
        ...
        for batch_idx in range(args.eval_step):
            ...
            if args.amp:
                with amp.autocast():
                    logits, masks_pred = model(inputs_x)
            else:
                logits, masks_pred = model(inputs_x)

            loss_class = cross_entropy_loss(logits, targets_x)
            loss_lovasz = lovasz_hinge(masks_pred.squeeze(), masks_x.squeeze())
            loss_mask_ce = F.binary_cross_entropy_with_logits(masks_pred.squeeze(), masks_x.squeeze())
            alpha = 0.8
            loss_mask = alpha * loss_lovasz + (1. - alpha) * loss_mask_ce
            loss = loss_class + loss_mask     # TODO; make lambda to control balance
            ...
            losses_lovasz.update(loss_lovasz.item())
            losses_mask_ce.update(loss_mask_ce.item())
            ...
            if not args.no_progress:
                p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. Cls: {loss_class:.4f}. Mask: {loss_mask:.4f}. Lovasz: {loss_lovasz:.4f}.".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    batch=batch_idx + 1,
                    iter=args.eval_step,
                    ...
                    # loss_mask=losses_mask.avg,
                    loss_lovasz=losses_lovasz.avg,
                    loss_mask=losses_mask_ce.avg,
                ))
                p_bar.update()
        ...
        if args.local_rank in [-1, 0]:
            test_loss, test_acc, test_map, test_loss_class, test_loss_lovasz, test_loss_mask_ce = test(args, test_loader, test_model, epoch)

            args.writer.add_scalar('train/1.train_loss', losses.avg, epoch)
            args.writer.add_scalar('train/2.train_loss_x', losses_class.avg, epoch)
            args.writer.add_scalar('train/4.train_loss_m', losses_mask.avg, epoch)
            args.writer.add_scalar('train/6.train_loss_lovasz', losses_lovasz.avg, epoch)
            args.writer.add_scalar('train/7.train_loss_mask_ce', losses_mask_ce.avg, epoch)
            args.writer.add_scalar('test/1.test_map', test_map, epoch)
            args.writer.add_scalar('test/2.test_loss', test_loss, epoch)
            args.writer.add_scalar('test/3.test_acc', test_acc, epoch)
            args.writer.add_scalar('test/4.test_loss_class', test_loss_class, epoch)
            args.writer.add_scalar('test/5.test_loss_lovasz', test_loss_lovasz, epoch)
            args.writer.add_scalar('test/6.test_loss_mask_ce', test_loss_mask_ce, epoch)
...
def test(args, test_loader, model, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, masks, targets) in enumerate(test_loader):
            ...
            loss_mask_ce = F.binary_cross_entropy_with_logits(masks_pred.squeeze(), masks.squeeze())
            loss_lovasz = lovasz_hinge(masks_pred.squeeze(), masks.squeeze())
            ...
            losses_class.update(loss_class.item(), inputs.shape[0])
            losses_mask.update(loss_mask.item(), inputs.shape[0])
            losses_lovasz.update(loss_lovasz.item(), inputs.shape[0])
            losses_mask_ce.update(loss_mask_ce.item(), inputs.shape[0])
            ...
            if not args.no_progress:
                test_loader.set_description("Test Iter: {batch:4}/{iter:4}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. Cls: {loss_class:.4f}. Mask: {loss_mask:.4f}. top1: {top1:.2f}.".format(
                    batch=batch_idx + 1,
    ...
    return losses.avg, top1.avg, mean_ap.mean(), losses_class.avg, losses_lovasz.avg, losses_mask_ce.avg
