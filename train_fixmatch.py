...
from fixmatch.datasets import (
    set_image_size,
    get_siim_data, get_unlabeled_data,
    get_transform_labeled, get_transform_valid, get_transform_unlabeled
)
...
def main():
    ...
    args.out = os.path.join(args.out, f'fixmatch-{datetime.now().strftime("%Y%m%d-%H%M%S")}')
    ...
    labeled_dataset = get_siim_data(args, validation=False, transform=get_transform_labeled())
    test_dataset = get_siim_data(args, validation=True, transform=get_transform_valid())
    ...
    if args.msd:
        model = attach_multi_stage_dropout(model, args.num_classes)
