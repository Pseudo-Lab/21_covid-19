...
from misc.loss import lovasz_hinge
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            inputs = torch.cat([inputs_x, inputs_u_s], 0)
            ...
            # TODO; weighted sum with focal loss ?
            #  https://github.com/yelanlan/Pneumothorax-Segmentation-2nd-place-solution/blob/bf99230deffdd813fe730ddeaed6822ba37193df/src_unet_cls/train_model.py#L143

            # TODO;
            lovasz_loss = lovasz_hinge(masks_pred_x, masks_x)
            mask_ce_loss = F.binary_cross_entropy_with_logits(masks_pred_x, masks_x)
            # Lm = 0.1 * lovasz_loss + 0.9 * mask_ce_loss
            # Lm = lovasz_loss
            Lm = mask_ce_loss
            ...
            # if torch.sum(mask) > 0:
            #     Lu = Lu / torch.sum(mask) * len(mask)  # normalize
            ...
            if args.amp:
                ...
                # torch.nn.utils.clip_grad_norm_(model.parameters(), 10.)  # TODO;
            ...
            losses_lovasz.update(lovasz_loss.item())
            losses_mask_ce.update(mask_ce_loss.item())
            scheduler.step()
            if args.use_ema:
            ...
            if not args.no_progress:
                p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. L_x: {loss_x:.4f}. L_u: {loss_u:.4f}. L_m: {loss_m:.4f}. L_l: {loss_lovasz:.4f}. L_m_ce: {loss_mask_ce:.4f}. Mask: {mask:.2f}. ".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    batch=batch_idx + 1,
                    iter=args.eval_step,
                    ...
                    loss_mask_ce=losses_mask_ce.avg,
                    mask=mask_probs.avg))
                p_bar.update()
...
def test(args, test_loader, model, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, masks, targets) in enumerate(test_loader):
            ...
            # loss_mask = 0.1 * loss_lovasz + 0.9 * loss_mask_ce
            # loss_mask = loss_lovasz
            loss_mask = loss_mask_ce
            ...
            if not args.no_progress:
                test_loader.set_description("Test Iter: {batch:4}/{iter:4}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. L_x: {loss_class:.4f}. L_l: {loss_lovasz:.4f}. L_m: {loss_mask_ce:.4f}. top1: {top1:.2f}.".format(
                    batch=batch_idx + 1,
